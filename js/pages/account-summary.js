...
export async function init(root, ctx) {
  ...
  root.querySelector('#asAccountsList').innerHTML = `
    ...
    <button class="as-acct-row w-full bg-white dark:bg-[#0D1728] border border-transparent dark:border-white/[0.06] rounded-[16px] p-[14px] shadow-lg flex items-center justify-between text-left cursor-pointer" data-acct="credit"><span class="text-[13px] font-semibold text-[#111827] dark:text-white">Verceil Signature Card</span><span class="flex items-center gap-[6px]"><span class="text-[13px] font-semibold text-[#6B7280] dark:text-[#8E9CBA]">Apply</span><svg class="w-[16px] h-[16px] text-[#6B7280] dark:text-[#52607D] flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><polyline points="9 18 15 12 9 6"></polyline></svg></span></button>
  `;
